...
class DocumentKNPAnnotator(override val name: String, override val props: Properties)
  ...
  def mkLocalAnnotator = new DocumentKNPLocalAnnotator

  class DocumentKNPLocalAnnotator extends LocalAnnotator with BaseKNPLocalAnnotator {
    ...
    override def newDocumentAnnotation(document: Node): Node = {
      ...
      val annotatedSentences: NodeSeq =
        sentenceNodes.zipWithIndex map { case (sentenceNode, idx) =>
          ...
          val result = runKNP(sentenceNode, Some(docIdInfo))
