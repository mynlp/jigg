...
class Pipeline(val props: Properties = new Properties) {
  ...
  def run = {
    ...
    val root = fn match {
      ...
      case fn => rootXML(jigg.util.IOUtil.openIterator(fn).mkString("\n"))
    }
  ...
  def shell = {
    ...
    val inputReader = jigg.util.IOUtil.openStandardIn
    def readLine: String = {
      System.err.print("> ")
    ...
    var in = readLine
    try while (in != "") {
      val xml = annotate(rootXML(in), annotators)
      val printer = new scala.xml.PrettyPrinter(500, 2)
  ...
  def annotate(input: String): Node = annotate(rootXML(input), createAnnotators)
  ...
  def rootXML(raw: String) = <root><document id={ newDocumentID() }>{ raw }</document></root>
}
