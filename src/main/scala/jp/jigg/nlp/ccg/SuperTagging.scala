...
trait SuperTagging extends Problem { outer =>
  ...
  override def save = {
    ...
    val os = jp.jigg.util.IOUtil.openBinOut(OutputOptions.saveModelPath)
    saveModel(os)
    os.close
  }
  ...
  def load = {
    import java.io._
    val in = jp.jigg.util.IOUtil.openBinIn(InputOptions.loadModelPath)
    loadModel(in)
    in.close
  }
  def loadModel(in: ObjectInputStream) = jp.jigg.util.LogUtil.track("Loading supertagger ...") {
